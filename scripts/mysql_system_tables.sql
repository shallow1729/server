...
-- This creates the mariadb.sys with standard defaults
REPLACE INTO global_priv (Host,User,Priv) VALUES ('localhost','mariadb.sys','{"access":0,"plugin":"mysql_native_password","authentication_string":"","account_locked":true,"password_last_changed":0}');
...
# Ensure mariadb.sys always has privileges on global_priv. If not the
# sys schema will not work

DELIMITER //
IF @skip_grants = 0 THEN
 -- Ensure we have select privilege, don't add delete if there is already some
 -- privileges
   set @rows= EXISTS (select * from mysql.tables_priv where host="localhost" and user="mariadb.sys");
  IF @rows = 0 THEN
  ...
  END IF;
ELSE
  INSERT IGNORE INTO tables_priv (Host,Db,User,Table_name,Grantor,Timestamp,Table_priv) VALUES ('localhost','mysql','mariadb.sys','global_priv','root@localhost','0','Select,Delete');
END IF //
DELIMITER ;
